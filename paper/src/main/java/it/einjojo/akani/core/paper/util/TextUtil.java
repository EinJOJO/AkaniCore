package it.einjojo.akani.core.paper.util;
...
public class TextUtil {
    private static final Map<String, String> AMPERSAND_TO_MINIMESSAGE;
     ...
     */
    public static String transformAmpersandToMiniMessage(String s) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (c == '&' && i < s.length() - 1) {
                String code = "&" + s.charAt(i + 1);
